...
class RespondKitScreenTest {
    ...
    private class Api : RespondKitApi {
        ...
        var readCount = 0
        override suspend fun createSession(installationId: String, context: CustomerContext, identityToken: String?) = ClientSession("session", "token_1234567890123456", "visitor", "2099-01-01T00:00:00Z")
        override suspend fun statuses(token: String, after: String?) = StatusPage(thread?.let { listOf(ThreadStatus(it, messages.size.toString())) }.orEmpty())
        override suspend fun createThread(token: String, clientThreadId: String): SupportThread {
            val value = SupportThread("thread_test", clientThreadId, "open", "2026-09-17T00:00:00Z", "2026-09-17T00:00:00Z")
            thread = value; return value
        }
        override suspend fun messages(token: String, threadId: String, after: String) = MessagePage(threadId, messages.drop(after.toInt()), messages.size.toString(), false)
        override suspend fun send(token: String, threadId: String, clientMessageId: String, text: String): Acceptance {
            val sent = SupportMessage("message_sent", threadId, clientMessageId, "customer_to_operator", text, acceptedAt = "2026-09-17T00:00:00Z", state = "available")
            messages += sent
            messages += SupportMessage("message_reply", threadId, direction = "operator_to_customer", text = "We can help", acceptedAt = "2026-09-17T00:00:01Z", state = "available")
            return Acceptance(sent.id, clientMessageId, "available", sent)
        }
        override suspend fun markRead(token: String, threadId: String, cursor: String) { readCount++ }
        override suspend fun logout(token: String) {}
    }
    @Test fun customTriggerFullScreenReadAndDraftPersistence() {
        val api = Api()
        lateinit var store: RespondKitStore
        compose.setContent {
            val scope = rememberCoroutineScope()
            store = remember { RespondKitStore(RespondKitConfiguration("https://support.example.com", "inbox_test", "https://example.com"), persistence = MemoryPersistence(), api = api, scope = scope) }
            ...
            MaterialTheme {
                if (showing) RespondKitScreen(store, onClose = { showing = false })
                    ...
                    Button(onClick = { showing = true }, modifier = Modifier.testTag("host-trigger")) { Text("Ask us") }
    ...
    @Test fun networkResponseBodyIsReadOffMainThread() = runBlocking {
        okhttp3.mockwebserver.MockWebServer().use { server ->
            server.start()
            server.enqueue(okhttp3.mockwebserver.MockResponse().setBody("{\"threads\":[]}"))
            val client = RespondKitClient(RespondKitConfiguration(server.url("/").toString(), "inbox_test", "https://example.com"))
