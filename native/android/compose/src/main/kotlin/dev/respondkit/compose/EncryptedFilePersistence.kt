...
/** Atomic, Android Keystore-encrypted state, excluded from cloud backup. A lost key is a surfaced error. */
class EncryptedFilePersistence(context: Context, scope: String) : RespondKitPersistence {
    private val digest = MessageDigest.getInstance("SHA-256").digest(scope.toByteArray()).joinToString("") { "%02x".format(it) }
    ...
    override fun load(): String? {
        val bytes = try { file.readFully() } catch (error: java.io.FileNotFoundException) { return null }
