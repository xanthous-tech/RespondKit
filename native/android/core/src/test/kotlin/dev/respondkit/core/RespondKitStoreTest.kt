...
private inline fun <reified T> fixture(key: String): T {
    val root =
        json
            .parseToJsonElement(
                File(System.getProperty("respondkit.fixtures"), "protocol.json").readText()
            )
            .jsonObject
...
private class FakeApi : RespondKitApi {
    ...
    override suspend fun createSession(
        ...
        sessionCalls++
        return fixture<SessionResponse>("session").session
    }

    override suspend fun statuses(token: String, after: String?): StatusPage {
        statusesEntered.complete(Unit)
        waitForStatuses?.await()
        if (unauthorizedOnce) {
            unauthorizedOnce = false
            throw RespondKitException("Expired", status = 401)
        }
    ...
    override suspend fun messages(token: String, threadId: String, after: String) =
        if (threadId == "thread_one") messagePage
        else MessagePage(threadId, emptyList(), "0", false)
...
class RespondKitStoreTest {
    private fun TestScope.store(api: FakeApi, storage: MemoryPersistence = MemoryPersistence()) =
        RespondKitStore(
    ...
    fun contractAndCursorValidation() {
        ...
        listOf("01", "-1", "1.2", "9007199254740992", "", "１２").forEach {
            assertFailsWith<RespondKitException> { replyCursor(it) }
        }
        assertEquals("acceptance_unknown", fixture<SendResponse>("acceptance").acceptance.status)
    }

    @Test
    fun unreadWithoutMountingScreenSurvivesOfflineRestart() = runTest {
        val api = FakeApi().apply { seed() }
        val storage = MemoryPersistence()
        val store = store(api, storage)
        store.refresh()
        assertEquals(2, store.state.value.statuses.size)
        assertTrue(store.state.value.hasUnreadReplies)
        assertTrue(api.reads.isEmpty())
        api.failStatuses = true
        val restored = store(api, storage)
        restored.refresh()
        assertTrue(restored.state.value.hasUnreadReplies)
        assertNotNull(restored.state.value.errorMessage)
    }

    @Test
    fun openingHistoryDoesNotReadAndReadingOneThreadKeepsOtherBadge() = runTest {
        val api = FakeApi().apply { seed() }
        val store = store(api)
        store.setForeground(true)
        store.setScreenVisible(true)
    ...
    @Test
    fun staleCursorAndBackgroundDoNotRead() = runTest {
        val api = FakeApi().apply { seed() }
        val store = store(api)
        store.setScreenVisible(true)
        store.selectThread("thread_one")
        ...
        assertTrue(api.reads.isEmpty())
        store.setForeground(true)
        store.markDisplayed("thread_one", "2")
        assertTrue(api.reads.isEmpty())
        store.setForeground(false)
    }

    @Test
    fun failedReadAckPersistsAndRetriesWithoutRestoringDot() = runTest {
        val api =
            FakeApi().apply {
                seed()
                failRead = true
    ...
    @Test
    fun uncertainSendRetriesImmutablePayloadAfterRestart() = runTest {
        ...
        assertEquals("Different draft", restored.state.value.draft)
        assertEquals(1, api.created.size)
    }

    @Test
    fun unauthorizedRenewsExactlyOnce() = runTest {
        val api =
            FakeApi().apply {
                seed()
        ...
        store.refresh()
        assertTrue(store.state.value.hasUnreadReplies)
        assertEquals(2, api.sessionCalls)
    }

    @Test
    fun accountSwitchRejectsLateResponses() = runTest {
        val api =
            FakeApi().apply {
                seed()
                waitForStatuses = CompletableDeferred()
            }
        val store = store(api)
        val request = launch { store.refresh() }
        api.statusesEntered.await()
        store.updateIdentity(CustomerContext(userId = "bob"))
        api.waitForStatuses?.complete(Unit)
        request.join()
        assertTrue(store.state.value.statuses.isEmpty())
        assertFalse(store.state.value.hasUnreadReplies)
    }

    @Test
    fun messageRevisionReplacesExistingRow() = runTest {
        val api = FakeApi().apply { seed() }
        val store = store(api)
        store.setScreenVisible(true)
        store.selectThread("thread_one")
        store.refresh()
        api.messagePage =
            api.messagePage.copy(
                messages = api.messagePage.messages.map { it.copy(text = "Revised") },
                nextCursor = "11",
            )
        store.refresh()
        assertEquals(1, store.state.value.messages.size)
        assertEquals("Revised", store.state.value.messages.single().text)
    }

    @Test
    fun repeatedForegroundDoesNotStartDuplicatePollingAndBackgroundStopsIt() = runTest {
        val api = FakeApi().apply { seed() }
        val store = store(api)
        store.setForeground(true)
        store.setForeground(true)
    ...
    @Test
    fun failedCanonicalMessageCanStillBeRetried() = runTest {
        val api = FakeApi().apply { seed() }
        api.messagePage =
            MessagePage(
                "thread_one",
        ...
        store.retry("cmsg_failed")
        assertEquals("cmsg_failed" to "Original", api.sent.single())
    }
}
