...
class RespondKitClientTest {
    @Test
    fun nativeOriginAndBearerAreSentAndMalformedResponsesFail() = runTest {
        MockWebServer().use { server ->
            ...
            assertEquals(
                401,
                assertFailsWith<RespondKitException> { client.statuses("token", null) }.status,
            )
        }
    }

    @Test
    fun mismatchedThreadResponsesAreRejected() = runTest {
        MockWebServer().use { server ->
            server.start()
            val client =
                RespondKitClient(
            ...
            server.enqueue(
                MockResponse()
                    .setBody(
                        """{"threadId":"someone_else","messages":[],"nextCursor":"0","hasMore":false}"""
                    )
            )
