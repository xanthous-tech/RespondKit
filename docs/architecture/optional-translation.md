...
2. In `apps/api/wrangler.jsonc`, set `env.staging.vars.TRANSLATION_ENABLED_INBOXES` to a JSON string such as `"[\"inbox_captioner_public\"]"`. The checked-in default is `"[]"` (disabled). Keep the existing `GEMINI_API_KEY` secret and a model available to that key. Local development uses `.dev.vars`. No new secret is required.
