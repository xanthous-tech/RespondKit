...
- Every message has an opaque public ID, and every customer-observable state revision has a D1 `INTEGER PRIMARY KEY AUTOINCREMENT` cursor. The widget pages transcript events by `(thread_id, row_id)` and polls every two seconds while the chat is open in a visible tab. A separate authenticated `/v1/thread-statuses` poll checks all authorized conversations every 10 seconds in visible tabs, including while the launcher is closed, to show an unread-reply dot.
- Customer read receipts use an authenticated `POST /v1/threads/:threadId/read` with an existing transcript cursor. The thread stores a monotonic cursor. Discord adds a bot-owned ✅ to the corresponding available-audit messages; successful reactions and retry times are persisted. A minute cron reconciles reads arriving before audit projection and transient REST failures. These are outbound reactions, requiring no Gateway.
